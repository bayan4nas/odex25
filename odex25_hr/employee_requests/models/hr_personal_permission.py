        ...
        for record in self:
            if record.date_from and record.date_to and record.employee_id:
                ...
                if calendar.is_full_day:
                    ...
                    cal_hour_end = calendar.full_max_sign_out
                    if cal_hour_end <= hour_end or hour_end < cal_hour_start:
                ...
                if item.balance and item.balance > 0:
                    if item.duration > item.balance:
                        raise exceptions.UserError(
                            _('This Duration must be less than or equal to the Permission Limit'))
                else:
                    if item.duration > 8:
                        raise exceptions.UserError(
                            _('This Duration must not exceed 8 hours when no Permission Limit is set'))
