        ...
        if self.employee_ids:
            if self.mission_type.journal_id:
                if self.Training_cost > 0:
                    invoice = self.env['account.move'].create({
                        ...
                        'invoice_line_ids': [(0, 0, {
                            ...
                            'item_budget_id': self.official_mission.item_budget_id.id,
                            'quantity': 1.0,
                ...
                for item in self.employee_ids:
                    budget_lines = self.official_mission.item_budget_id.crossovered_budget_line.filtered(
                    ...
                    if not budget_lines:
                        raise ValidationError(_('No budget for this service: {} - {}').format(
                            self.official_mission.name, self.official_mission.item_budget_id.name))
                    ...
                    general_budget = self.official_mission.item_budget_id.crossovered_budget_line.filtered(
                        lambda bl: bl.general_budget_id in self.env['account.budget.post'].search([]).filtered(
                            lambda post: self.official_mission.rule_debit_account_id in post.account_ids))
                    if not general_budget:
                        raise ValidationError(_('No budget for this account: {}').format(self.official_mission.rule_debit_account_id.name))
                    if item.amount > 0.0:
                        invoice_line_ids = [(0, 0, {
                            'name': self.official_mission.name,
                            'account_id': self.official_mission.rule_debit_account_id.id,
                            'analytic_account_id': self.official_mission.analytic_account_id.id,
                            'item_budget_id': self.official_mission.item_budget_id.id,
                            'quantity': 1.0,
