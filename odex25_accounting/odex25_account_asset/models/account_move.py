...
class AccountMove(models.Model):
    ...
    asset_manually_modified = fields.Boolean(
        help='This is a technical field stating that a depreciation line has been manually modified. It is used to recompute the depreciation table of an asset/deferred revenue.',
        copy=False)
    asset_value_change = fields.Boolean(
        help='This is a technical field set to true when this move is the result of the changing of value of an asset')
    asset_ids = fields.One2many('account.asset', string='Assets', compute="_compute_asset_ids")
    asset_ids_display_name = fields.Char(
        ...
        for move in self:
            ...
            for line in move.line_ids:
                for asset in line.asset_ids:
                    if asset.state == 'draft' or all(
                            state == 'draft' for state in asset.depreciation_move_ids.mapped('state')):
                        asset.state = 'draft'
                        asset.unlink()
    ...
    def _log_depreciation_asset(self):
        for move in self.filtered(lambda m: m.asset_id):
            asset = move.asset_id
            msg = _('Depreciation entry %s posted (%s)') % (
                move.name, formatLang(self.env, move.amount_total, currency_obj=move.company_id.currency_id))
    ...
    def _compute_asset_ids(self):
        for record in self:
            ...
            if record.number_asset_ids:
                ...
                record.asset_ids_display_name = '%s %s' % (
                    len(record.asset_ids), asset_type.get(record.asset_ids[0].asset_type))
            else:
                record.asset_ids_display_name = ''
            record.asset_id_display_name = {'sale': _('Revenue'), 'purchase': _('Asset'), 'expense': _('Expense')}.get(
                record.asset_id.asset_type)
...
class AccountMoveLine(models.Model):
    ...
    asset_ids = fields.Many2many('account.asset', 'asset_move_line_rel', 'line_id', 'asset_id', string='Asset Linked',
                                 help="Asset created from this Journal Item", copy=False)
    ...
    def turn_as_asset(self):
        return self._turn_as_asset('purchase', _("Turn as an asset"),
                                   self.env.ref("odex25_account_asset.view_account_asset_form"))

    def turn_as_deferred(self):
        balance = sum(aml.debit - aml.credit for aml in self)
        if balance > 0:
            return self._turn_as_asset('expense', _("Turn as a deferred expense"),
                                       self.env.ref('odex25_account_asset.view_account_asset_expense_form'))
        else:
            return self._turn_as_asset('sale', _("Turn as a deferred revenue"),
                                       self.env.ref('odex25_account_asset.view_account_asset_revenue_form'))
