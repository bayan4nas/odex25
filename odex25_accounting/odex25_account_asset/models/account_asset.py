...
class AccountAsset(models.Model):
    ...
    gross_increase_count = fields.Integer(compute='_entry_count', string='# Gross Increases',
    ...
    state = fields.Selection(
        [('model', 'Model'), ('draft', 'Draft'), ('open', 'Running'), ('paused', 'On Hold'), ('close', 'Closed')],
    ...
    # Depreciation params
    method = fields.Selection([('linear', 'Straight Line'), ('degressive', 'Declining'),
    ...
    method_number = fields.Integer(string='Number of Depreciations', readonly=True, tracking=True,
    ...
    method_period = fields.Selection([('1', 'Months'), ('12', 'Years')], string='Number of Months in a Period',
                                     ...
                                     help="The amount of time between two depreciations", tracking=True)
    method_progress_factor = fields.Float(string='Declining Factor', readonly=True, default=0.3,
                                          states={'draft': [('readonly', False)], 'model': [('readonly', False)]})
    prorata = fields.Boolean(string='Prorata Temporis', readonly=True, tracking=True,
                             states={'draft': [('readonly', False)], 'model': [('readonly', False)]},
    ...
    model_id = fields.Many2one('account.asset', string='Model', change_default=True, readonly=True,
                               states={'draft': [('readonly', False)]}, domain="[('company_id', '=', company_id)]")
    user_type_id = fields.Many2one('account.account.type', related="account_asset_id.user_type_id",
    ...
    children_ids = fields.One2many('account.asset', 'parent_id',
    ...
    already_depreciated_amount_import = fields.Monetary(
        help="In case of an import from another software, you might need to use this field to have the right depreciation table report. This is the value that was already depreciated with entries not computed from this model",
        default=0.0)
    depreciation_number_import = fields.Integer(
        help="In case of an import from another software, provide the number of depreciations already done before starting with Odoo.",
        default=0)
    ...
    def _compute_value(self):
        for record in self:
            ...
            if any(type != record.original_move_line_ids[0].move_id.move_type for type in
                   record.original_move_line_ids.mapped('move_id.move_type')):
    ...
    def _compute_acquisition_date(self):
        for record in self:
            record.acquisition_date = record.acquisition_date or min(record.original_move_line_ids.mapped('date') + [
    ...
    def _compute_value_residual(self):
        for record in self:
            record.value_residual = record.original_value - record.salvage_value - record.already_depreciated_amount_import - abs(
                sum(record.depreciation_move_ids.filtered(lambda m: m.state == 'posted').mapped('amount_total')))
    ...
    def _compute_first_depreciation_date(self):
        for record in self:
            pre_depreciation_date = record.acquisition_date or min(record.original_move_line_ids.mapped('date'),
                                                                   default=record.acquisition_date) or fields.Date.today()
    ...
    def compute_depreciation_board(self):
        ...
        amount_change_ids = self.depreciation_move_ids.filtered(
            lambda x: x.asset_value_change and not x.reversal_move_id).sorted(key=lambda l: l.date)
        # print(self.asset_value_change, 'x.asset_value_change')
        ...
        if amount_to_depreciate != 0.0:
            for asset_sequence in range(starting_sequence + 1, depreciation_number + 1):
                ...
                move_ref = self.name + ' (%s/%s)' % (
                    prorata and asset_sequence - 1 or asset_sequence, self.method_number)
        ...
        for asset, invoice_line_id in zip(self, invoice_line_ids):
            ...
            if unposted_depreciation_move_ids:
                ...
                depreciated_amount = copysign(
                    sum(asset.depreciation_move_ids.filtered(lambda r: r.state == 'posted').mapped('amount_total')),
                    -initial_amount)
    ...
    def set_to_running(self):
        if self.depreciation_move_ids and not max(self.depreciation_move_ids,
                                                  key=lambda m: m.date).asset_remaining_value == 0:
    ...
    def pause(self, pause_date):
        ...
        if following_lines:
            ...
            if self.prorata:
                ...
                try:
                    time_ratio = ((pause_date - depreciation_period_start).days) / (
                            first_following.date - depreciation_period_start).days
                    new_line = self._insert_depreciation_line(line_before_pause,
                                                              first_following.amount_total * time_ratio,
                                                              _("Asset paused"), pause_date)
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if 'state' in vals and vals['state'] != 'draft' and not (
                    set(vals) - set({'account_depreciation_id', 'account_depreciation_expense_id', 'journal_id'})):
