...
_is_used_simple_token_store = sys.modules.get('odoo.addons.takaful_rest_api.controllers.simple_token_store')


class RestApiAccessToken(models.Model):
    _name = "takaful.rest.api.access.token"
    _description = "Access Tokens ('takaful_rest_api')"
    ...
    def delete_expired_tokens_in_table(self, table):
        model_name = 'takaful.rest.api.' + table + '.token'
...
class RestApiRefreshToken(models.Model):
    _name = "takaful.rest.api.refresh.token"
    _description = "Refresh Tokens ('takaful_rest_api')"
