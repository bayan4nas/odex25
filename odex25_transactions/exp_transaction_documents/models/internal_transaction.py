...
class InternalTransaction(models.Model):
    ...
    def get_url(self):
        self.ensure_one()
        url = ''
        action = self.env.ref('exp_transaction_documents.incoming_internal_tran_action', False)
        ...
        url = f"{base_url}/web#id={self.id}&model=internal.transaction&action={action.id}"
