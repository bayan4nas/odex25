...
class OutgoingTransaction(models.Model):
    ...
    def get_url(self):
        self.ensure_one()
        url = ''
        action = self.env.ref('exp_transaction_documents.outgoing_external_tran_action', False)
        ...
        url = f"{base_url}/web#id={self.id}&model=outgoing.transaction&action={action.id}"
