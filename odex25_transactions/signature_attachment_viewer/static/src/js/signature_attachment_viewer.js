odoo.define('signature_attachment_viewer.signature_attachment_viewer', function (require) {
    "use strict";
    ...
    var DocumentViewer = require('signature_attachment_viewer.DocumentViewer');
...
_previewAttachment: function (ev) {
    ...
    var recordId = self.record && self.record.data && self.record.data.id;

    this._rpc({
        model: 'cm.attachment.rule',
        method: 'read',
        args: [[recordId], ['internal_transaction_id', 'incoming_transaction_id', 'outgoing_transaction_id']],
    }).then(function (rule_result) {
        var rule = rule_result[0];

        var internalId = rule.internal_transaction_id && rule.internal_transaction_id[0];
        var incomingId = rule.incoming_transaction_id && rule.incoming_transaction_id[0];
        var outgoingId = rule.outgoing_transaction_id && rule.outgoing_transaction_id[0];

        var fetchSignature = function (model, id) {
            return self._rpc({
                model: model,
                method: 'read',
                args: [[id], ['signature']],
        ...
        if (internalId) {
            promise = fetchSignature('internal.transaction', internalId);
        } else if (incomingId) {
            promise = fetchSignature('incoming.transaction', incomingId);
        } else if (outgoingId) {
            promise = fetchSignature('outgoing.transaction', outgoingId);
        }

        promise.then(function (signature) {
            if (!signature) {
                console.warn("❌ لا يوجد توقيع في أي من الحقول الثلاثة.");
                return;
            }
            ...
            self._rpc({
                ...
                method: 'read_as_sudo',
                kwargs: {
                    domain: [['id', 'in', self.value.res_ids]],
    ...
    field_registry.add("signature_attachment_viewer", FieldMany2ManyAttachmentPreview);
