...
class ContractLine(models.Model):
    ...
    contract_id = fields.Many2one('contract.contract', string='Contract', required=True, index=True, ondelete='cascade')
    analytic_account_id = fields.Many2one('account.analytic.account', string="Cost Center", )
    date_start = fields.Date('Date Start', required=True, default=lambda self: fields.Date.context_today(self), )
    date_end = fields.Date(string='Date End', index=True)
    recurring_next_date = fields.Date(string='Date of Next Invoice')
    last_date_invoiced = fields.Date(string='Last Date Invoiced', readonly=True, copy=False)
    termination_notice_date = fields.Date(string='Termination notice date', compute="_compute_termination_notice_date",
                                          store=True, copy=False)
    create_invoice_visibility = fields.Boolean(compute='_compute_create_invoice_visibility')
    successor_contract_line_id = fields.Many2one('contract.line', string="Successor Contract Line", required=False,
    ...
    predecessor_contract_line_id = fields.Many2one('contract.line', "Predecessor Contract Line", required=False,
                                                   readonly=True, index=True, copy=False,
                                                   help="Contract Line origin of this one.")
    manual_renew_needed = fields.Boolean(string="Manual renew needed", default=False,
                                         help="This flag is used to make a difference between a definitive stop and temporary one for which a user is not able to plan a successor in advance")
    ...
    state = fields.Selection(string="State", selection=[('upcoming', 'Upcoming'), ('in-progress', 'In-progress'),
                                                        ('to-renew', 'To renew'),
                                                        ('upcoming-close', 'Upcoming Close'), ('closed', 'Closed'),
                                                        ('canceled', 'Canceled')], compute="_compute_state",
                             search='_search_state', )
    active = fields.Boolean(string="Active", related="contract_id.active", store=True, readonly=True, default=True, )

    @api.depends('date_end', 'termination_notice_rule_type', 'termination_notice_interval')
    def _compute_termination_notice_date(self):
        for rec in self:
            if rec.date_end:
                rec.termination_notice_date = (
                        fields.Date.from_string(rec.date_end) - self.get_relative_delta(
    ...
    def _compute_state(self):
        ...
        for rec in self:
            ...
            if rec.date_start and rec.date_start <= today and (not rec.date_end or rec.date_end >= today):
                # In period
                if rec.termination_notice_date and rec.termination_notice_date < today and not rec.is_auto_renew and not rec.manual_renew_needed:
    ...
    def _get_state_domain(self, state):
        ...
        if state == 'in-progress':
            return ["&", "&", "&", ('date_start', '<=', today), ('is_canceled', '=', False), "|",
                    ('date_end', '>=', today), ('date_end', '=', False), "|", "&", ('is_auto_renew', '=', True),
                    ('is_auto_renew', '=', False), ('termination_notice_date', '>', today), ]
        if state == 'to-renew':
            return ["&", "&", ('is_canceled', '=', False), ('date_end', '<', today), "|", "&",
                    ('manual_renew_needed', '=', True), ('successor_contract_line_id', '=', False),
                    ('is_auto_renew', '=', True)]
        if state == 'upcoming-close':
            return ["&", "&", "&", "&", "&", ('date_start', '<=', today), ('is_auto_renew', '=', False),
                    ('manual_renew_needed', '=', False), ('is_canceled', '=', False),
                    ('termination_notice_date', '<', today), ('date_end', '>=', today)]
        if state == 'closed':
            return ["&", "&", "&", ('is_canceled', '=', False), ('date_end', '<', today), ('is_auto_renew', '=', False),
                    "|", "&", ('manual_renew_needed', '=', True), ('successor_contract_line_id', '!=', False),
    ...
    def _search_state(self, operator, value):
        states = ['upcoming', 'in-progress', 'to-renew', 'upcoming-close', 'closed', 'canceled']
    ...
    @api.depends('date_start', 'date_end', 'last_date_invoiced', 'is_auto_renew', 'successor_contract_line_id',
                 'predecessor_contract_line_id', 'is_canceled', )
    def _compute_allowed(self):
        for rec in self:
            if rec.date_start:
                allowed = get_allowed(rec.date_start, rec.date_end, rec.last_date_invoiced, rec.is_auto_renew,
                                      rec.successor_contract_line_id, rec.predecessor_contract_line_id, rec.is_canceled)
                if allowed:
                    rec.is_plan_successor_allowed = allowed.plan_successor
                    rec.is_stop_plan_successor_allowed = allowed.stop_plan_successor
    ...
    @api.model
    def _compute_first_recurring_next_date(self, date_start, recurring_invoicing_type, recurring_rule_type,
                                           recurring_interval):
        if recurring_rule_type == 'monthlylastday':
            return date_start + self.get_relative_delta(recurring_rule_type, recurring_interval - 1)
        if recurring_invoicing_type == 'pre-paid':
            return date_start
        return fields.Date.from_string(date_start) + self.get_relative_delta(recurring_rule_type, recurring_interval)

    @api.model
    def compute_first_date_end(self, date_start, auto_renew_rule_type, auto_renew_interval):
        return date_start + self.get_relative_delta(auto_renew_rule_type, auto_renew_interval) - relativedelta(days=1)

    @api.onchange('date_start', 'is_auto_renew', 'auto_renew_rule_type', 'auto_renew_interval', )
    def _onchange_is_auto_renew(self):
        ...
        for rec in self.filtered('is_auto_renew'):
            if rec.date_start:
                rec.date_end = self.compute_first_date_end(fields.Date.from_string(rec.date_start),
                                                           rec.auto_renew_rule_type, rec.auto_renew_interval)

    @api.onchange('date_start', 'recurring_invoicing_type', 'recurring_rule_type', 'recurring_interval')
    def _onchange_date_start(self):
        for rec in self.filtered('date_start'):
            rec.recurring_next_date = self._compute_first_recurring_next_date(rec.date_start,
            ...
            if line.date_start and line.recurring_next_date:
                if line.date_start > line.recurring_next_date:
                    raise ValidationError(
                        _("You can't have a date of next invoice anterior to the start of the contract line '%s'") % line.name)
    ...
    def _check_last_date_invoiced(self):
        for rec in self.filtered('last_date_invoiced'):
            if rec.date_start and rec.date_start > rec.last_date_invoiced:
                raise ValidationError(
                    _("You can't have the start date after the date of last invoice for the contract line '%s'") % rec.name)
            if rec.date_end and rec.date_end < rec.last_date_invoiced:
                raise ValidationError(
                    _("You can't have the end date before the date of last invoice for the contract line '%s'") % rec.name)
        ...
        for line in self.filtered('date_end'):
            if line.date_start and line.date_end:
                if line.date_start > line.date_end:
                    raise ValidationError(_("Contract line '%s' start date can't be later than end date") % line.name)
    ...
    def _prepare_invoice_line(self, invoice_id=False):
        ...
        invoice_line_vals.update(
            {'name': name, 'analytic_account_id': self.analytic_account_id.id, 'price_unit': self.price_unit})
    ...
    def _get_period_to_invoice(self, last_date_invoiced, recurring_next_date, stop_at_date_end=True):
        ...
        if self.recurring_rule_type == 'monthlylastday':
            ...
            if self.recurring_invoicing_type == 'pre-paid':
                last_date_invoiced = (fields.Date.from_string(recurring_next_date) + self.get_relative_delta(
                    self.recurring_rule_type, self.recurring_interval) - relativedelta(days=1))
            else:
                last_date_invoiced = fields.Date.from_string(recurring_next_date) - relativedelta(days=1)
        if stop_at_date_end:
    ...
    def _update_recurring_next_date(self, recurring_next_date=None):
        for rec in self:
            ...
            diff_date = self.get_relative_delta(rec.recurring_rule_type, rec.recurring_interval)
    ...
    def _init_last_date_invoiced(self):
        ...
        for rec in self:
            ...
            if rec.recurring_rule_type == 'monthlylastday':
                last_date_invoiced = (rec.recurring_next_date - self.get_relative_delta(rec.recurring_rule_type,
                                                                                        rec.recurring_interval))
            elif rec.recurring_invoicing_type == 'post-paid':
                last_date_invoiced = (rec.recurring_next_date - self.get_relative_delta(rec.recurring_rule_type,
                                                                                        rec.recurring_interval)) - relativedelta(
                    days=1)
    ...
    def _delay(self, delay_delta):
        ...
        for rec in self:
            ...
            rec.recurring_next_date = self._compute_first_recurring_next_date(new_date_start,
                                                                              rec.recurring_invoicing_type,
                                                                              rec.recurring_rule_type,
                                                                              rec.recurring_interval)
            if rec.date_end:
                ...
                if not rec.date_end or rec.date_end > date_end:
                    ...
                    if post_message:
                        msg = _(
                            ...
                            """.format(product=rec.name, old_end=old_date_end, new_end=rec.date_end, ))
                        rec.contract_id.message_post(body=msg)
                else:
                    rec.write({'is_auto_renew': False, "manual_renew_needed": manual_renew_needed})
        return True

    def _prepare_value_for_plan_successor(self, date_start, date_end, is_auto_renew, recurring_next_date=False):
        self.ensure_one()
        if not recurring_next_date:
            recurring_next_date = self._compute_first_recurring_next_date(date_start, self.recurring_invoicing_type,
    ...
    def plan_successor(self, date_start, date_end, is_auto_renew, recurring_next_date=False, post_message=True):
        """
        ...
        for rec in self:
            ...
            if post_message:
                msg = _(
                    ...
                    """.format(product=rec.name, new_date_start=new_line.date_start, new_date_end=new_line.date_end))
                rec.contract_id.message_post(body=msg)
    ...
    def stop_plan_successor(self, date_start, date_end, is_auto_renew):
        ...
        for rec in self:
            if rec.date_start >= date_start:
                ...
                elif rec.date_end and date_start < rec.date_end < date_end:
                    new_date_start = date_end + relativedelta(days=1)
                    new_date_end = (date_end + (rec.date_end - date_start) + relativedelta(days=1))
                    rec.stop(date_start - relativedelta(days=1), manual_renew_needed=True, post_message=False)
                    contract_line |= rec.plan_successor(new_date_start, new_date_end, is_auto_renew, post_message=False)
                else:
                    new_date_start = date_end + relativedelta(days=1)
                    if rec.date_end:
            ...
            msg = _(
                ...
                """.format(product=rec.name, new_date_start=date_start, new_date_end=date_end))
            rec.contract_id.message_post(body=msg)
        return contract_line
    ...
    def cancel(self):
        ...
        for contract in self.mapped('contract_id'):
            ...
            msg = _("""Contract line canceled: %s""" % "<br/>- ".join(
    ...
    def uncancel(self, recurring_next_date):
        ...
        for contract in self.mapped('contract_id'):
            lines = self.filtered(lambda l, c=contract: l.contract_id == c)
            msg = _("""Contract line Un-canceled: %s""" % "<br/>- ".join(
                ["<strong>%s</strong>" % name for name in lines.mapped('name')]))
            contract.message_post(body=msg)
        for rec in self:
            if rec.predecessor_contract_line_id:
    ...
    def action_uncancel(self):
        self.ensure_one()
        context = {'default_contract_line_id': self.id, 'default_recurring_next_date': fields.Date.context_today(self)}
    ...
    def action_plan_successor(self):
        self.ensure_one()
        context = {'default_contract_line_id': self.id, 'default_is_auto_renew': self.is_auto_renew}
        context.update(self.env.context)
    ...
    def action_stop(self):
        self.ensure_one()
        context = {'default_contract_line_id': self.id, 'default_date_end': self.date_end}
        context.update(self.env.context)
    ...
    def _get_renewal_dates(self):
        ...
        date_end = self.compute_first_date_end(date_start, self.auto_renew_rule_type, self.auto_renew_interval)
    ...
    def renew(self):
        ...
        for rec in self:
            ...
            new_line = rec.plan_successor(date_start, date_end, is_auto_renew, post_message=False)
