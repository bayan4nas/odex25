...
class ContractInstallmentLine(models.Model):
    ...
    tax_amount = fields.Float(compute='_compute_amount_tax', string='Tax Amount', readonly=True, store=True)
    total_amount = fields.Float(compute='_compute_total_amount', store=True, string='Amount With Tax', readonly=True)
    untax_amount = fields.Float(compute="_compute_untax_amount",strore=True, string="Untax Amount", readonly=True )
    due_date = fields.Date(string='Due Date', index=True, required=True)
    state = fields.Selection([('not_invoiced', 'Not Invoiced'), ('invoiced', 'Invoiced'), ('paid', 'Paid'), ('cancel', 'Cancel')],
                             string="Status", default="not_invoiced")
    ...
    contract_id = fields.Many2one('contract.contract', string='Contract', ondelete='restrict', index=True,)
    installment_type = fields.Selection(string='Type', selection=[('percent', 'Percent'), ('fixed', 'Fixed')], default="fixed", required=True)
    percent = fields.Float(string='Percent')
    partner_id = fields.Many2one(string='partner', index=True, comodel_name='res.partner', readonly=True, related="contract_id.partner_id")
    company_id = fields.Many2one(string='Company', comodel_name='res.company', readonly=True, related="contract_id.company_id")
    user_id = fields.Many2one('res.users', string='User', readonly=True, related="contract_id.user_id", copy=False)
    ...
    taxed_invoice = fields.Selection([('taxed_invoice', 'Taxed Invoice'), ('final_invoice', 'Final invoice (reconciliation  advance payment and tax )')],
                                     string=" Taxed Invoice", default="taxed_invoice")
    ...
    contract_date = fields.Date(related='contract_id.date')

    @api.constrains('contract_date', 'due_date')
    def _check_due_date(self):
        if self.due_date and self.contract_date and self.due_date < self.contract_date:
            raise ValidationError(_('Due Date must be greate than or equal to Contract Date'))
    ...
    def onchange_total_amount(self):
        installments = sum(self.search([('id', '!=', self._origin.id), ('contract_id', '=', self.contract_id.id), ('state', '!=', 'cancel')]).mapped('total_amount'))
    ...
    def _onchange_installment_type(self):
        ...
        for rec in self:
            if rec.percent > 0:
                rec.amount = rec.contract_id.with_tax_amount * rec.percent / 100
            ...
            if rec.taxed_invoice == 'final_invoice':
                if rec.contract_id:
                    ...
                    installments = self.search([('contract_id', '=', self.contract_id.id), ('taxed_invoice', 'in', [
                                               'taxed_invoice', 'with_invoice']), ('state', '=', 'invoiced')])
                    amount = sum(installments.mapped('amount'))
                    if amount:
                        rec.amount = rec.contract_id.with_tax_amount - amount
                    # rec.amount = rec.contract_id.with_tax_amount
                    # rec.tax_id = rec.contract_id.tax_id if rec.contract_id.tax_id else False
                    # rec.tax_amount = rec.contract_id.tax_amount
                    # rec.with_tax_amount = rec.contract_id.with_tax_amount
    ...
    def _compute_amount_tax(self):
        ...
        for rec in self:
            if rec.tax_id:
                taxes = [tax._compute_amount(
                    rec.amount, rec.amount) for tax in self.tax_id]
                rec.tax_amount = rec.amount - sum(taxes)
    ...
    def _compute_untax_amount(self):
        for rec in self:
            rec.untax_amount = 0.0
            if rec.tax_amount:
                rec.untax_amount = rec.amount - rec.tax_amount

    @api.model
    def send_contract_email(self):

        # add 30 days to the day the cron running to compare with due date
        ...
        if contract:
            ...
            if contract_id.state == 'closed':
                raise ValidationError(
                    _("Contract is closed you cant create installments for it!"))
