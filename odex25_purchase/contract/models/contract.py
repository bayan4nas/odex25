...
class ContractContract(models.Model):
    ...
    _order = "id desc"
    _inherit = ["mail.thread", "mail.activity.mixin", "contract.abstract.contract"]

    purchase_id = fields.Many2one("purchase.order", string="Purchase Order")
    request_id = fields.Many2one("purchase.request", "Request Ref.", related="purchase_id.request_id")
    department_id = fields.Many2one(related='purchase_id.department_id')
    active = fields.Boolean(default=True)
    code = fields.Char(string="Reference")
    group_id = fields.Many2one(string="Group", comodel_name="account.analytic.account", ondelete="restrict")
    currency_id = fields.Many2one(related="company_id.currency_id", string="Currency", readonly=True)
    contract_template_id = fields.Many2one(string="Contract Template", comodel_name="contract.template")
    contract_line_ids = fields.One2many(string="Contract lines", comodel_name="contract.line",
                                        inverse_name="contract_id", copy=True)
    ...
    recurring_next_date = fields.Date(compute="_compute_recurring_next_date", string="Date of Next Invoice", store=True)
    date_end = fields.Date(compute="_compute_date_end", string="Date End", store=True, readonly=False)
    ...
    fiscal_position_id = fields.Many2one(comodel_name="account.fiscal.position", string="Fiscal Position",
                                         ondelete="restrict")
    invoice_partner_id = fields.Many2one(string="Invoicing contact", comodel_name="res.partner", ondelete="restrict")
    partner_id = fields.Many2one(comodel_name="res.partner", inverse="_inverse_partner_id", required=True)
    commercial_partner_id = fields.Many2one("res.partner", related="partner_id.commercial_partner_id", store=True,
                                            string="Commercial Entity", index=True)
    type_of_contraction = fields.Selection(selection=[("subs", "Subscription"), ("contract", "Legal Contract")],
                                           string="Contraction Type", default="contract")
    installment_count = fields.Integer(compute="compute_installment_count")
    type = fields.Selection(related="contract_template_id.journal_id.type", string="Type", readonly=True)
    ...
    total_amount = fields.Float(compute="_compute_amount", string="Total Amount", store=True, readonly=True)
    date_start = fields.Date(string="Date Start")
    note = fields.Html(string="Note")
    tax_id = fields.Many2one(comodel_name="account.tax", string="Taxes")
    tax_amount = fields.Float(compute="_compute_amount", string="Tax Amount", readonly=True, store=True)
    with_tax_amount = fields.Float(compute="_compute_amount", store=True, string="Amount With Tax", readonly=True)
    ...
    project_period = fields.Integer(string="Project Duration", compute='_compute_project_period')
    date = fields.Date(string="Date")
    penalty = fields.Float(string="Penalty Amount")
    penalty_type = fields.Selection(string="Penalty type", selection=[("fix", "Fixed"), ("percent", "Percent")],
                                    default="fix")
    percent_amount = fields.Float(string="percent")
    is_penalty = fields.Boolean(string="Penalty")
    name_seq = fields.Char(string="Contract No", default="/", readonly=True, copy=False)
    analytic_account_id = fields.Many2one(string="Cost Center", comodel_name="account.analytic.account",
                                          compute="get_line_analytic_account")
    remaining_amount = fields.Float(string="Remaining", compute="_compute_installments")
    ...
    def _compute_project_period(self):
        for record in self:
            if record.date_start and record.date_end:
                ...
                record.project_period = delta.years * 12 + delta.months + (1 if delta.days > 0 else 0)
            else:
                record.project_period = 0.0

            record.get_recurring_next_date()

    def _compute_installments(self):
        for record in self:
            installments = sum(self.env["account.payment"].search([("contract_id", "=", record.id)]).mapped("amount"))
    ...
    def installment_table(self):
        ...
        for rec in installment:
            instellment_table += (
                        "<tr style='border: 1px solid gray'><td style='width:5%;border: 1px solid gray;background-color:#002d4d;color:white;'>" + rec.name + "</td>")
            ...
            if rec.description:
                instellment_table += (
                            "<td style='width:40%;;border: 1px solid gray;word-break:break-all'>" + rec.description + "</td></tr>")
            else:
    ...
    def get_contract_content(self, content):
        ...
        body = (
            content.replace("second party name", self.partner_id.name)
            .replace("installment", installment)
            .replace("hdate", str(hdate))
            .replace("date", self.date if self.date else "")
            .replace(
                "first party name",
                self.first_party.name if self.first_party.name else "",
            )
            .replace(
                "commercial",
                self.first_party.commercial_register
                if self.first_party.commercial_register
                else "",
            )
            .replace(
                "address", self.first_party.street if self.first_party.street else ""
            )
            .replace(
                "fCEO",
                self.first_party.executive_manager
                if self.first_party.executive_manager
                else "",
            )
            .replace(
                "saddress", self.partner_id.street if self.partner_id.street else ""
            )
            .replace(
                "scommercial",
                self.partner_id.commercial_register
                if self.partner_id.commercial_register
                else "",
            )
            .replace(
                "sCEO",
                self.partner_id.executive_manager
                if self.partner_id.executive_manager
                else "",
            )
            .replace(
                "specialist",
                self.partner_id.specialist if self.partner_id.specialist else "",
            )
            .replace("SOname", "")
            .replace("SOD", "")
            .replace("POname", "")
            .replace("POD", "")
            .replace("total", str(self.total_amount))
            .replace("letter", total_letter)
            .replace("project period", str(self.project_period))
            .replace("Pperiod", project_period_letter)
            .replace("Fresponsible", "")
            .replace("Femail", "")
            .replace(
                "Sresponsible",
                self.partner_id.appointed if self.partner_id.appointed else "",
            )
            .replace(
                "Semail", self.partner_id.app_email if self.partner_id.app_email else ""
            )
        )
        return body
    ...
    def suspended_state(self):
        ...
        try:
            ...
            author_id = self.env.user.partner_id.id or None
            self.partner_id.send_notification_message(
                subject=subject, body=message, author_id=author_id
            )
        except Exception as e:
            pass
    ...
    def closed_state(self):
        if self.remaining_amount > 0 :
            raise ValidationError(_("contract cannot be closed due to reamaining amount greate than zero"))
    ...
    def _get_related_invoices(self):
        self.ensure_one()
        invoices = self.env["account.move"].search([("contract_id", "=", self.id), ('move_type', '=', 'in_invoice')])
        return invoices
    ...
    def action_show_invoices(self):
        ...
        form_view_ref = ("account.move_supplier_form" if self.contract_type == "purchase" else "account.move_form")
    ...
    @api.depends("contract_line_ids.recurring_next_date", "contract_line_ids.is_canceled")
    @api.model
    def get_recurring_next_date(self):
        recurring_next_date = self.contract_line_ids.filtered(
            lambda l: l.recurring_next_date and not l.is_canceled).mapped("recurring_next_date")
    ...
    def _onchange_contract_template_id(self):
        ...
        for field_name, field in contract_template_id._fields.items():
            ...
            elif not any((field.compute, field.related, field.automatic, field.readonly, field.company_dependent,
        ...
        for i in contracts:
            if i.date_end:
                ...
                if date_now >= exp_date:
                    mail_content = (
                            "Hello, Mr. " + str(i.user_id.name) + ". Notice about the end date of the contract: " +
                            "name of contract " + str(i.name) + " for customer " + str(i.partner_id.name)
                    )
                    main_content = {
        ...
        if not journal:
            journal = (
                ...
                if self.journal_id.type == self.contract_type
                    ...
                    [("type", "=", self.contract_type), ("company_id", "=", self.company_id.id), ], limit=1))
        if not journal:
    ...
    def _finalize_and_create_invoices(self, invoices_values):
        ...
        invoices = (
            ...
            .create(final_invoices_values[0])
        )
        for invoice in invoices:
            for line in invoice.invoice_line_ids:
                line.tax_ids = line.contract_line_id.tax_id and [
                    (4, line.contract_line_id.tax_id.id)
                ]
    ...
    def _get_lines_to_invoice(self, date_ref):
        ...
        return self.contract_line_ids.filtered(
            lambda l: not l.is_canceled and l.recurring_next_date and l.recurring_next_date <= date_ref)
    ...
    def _compute_amount(self):
        for rec in self:
            rec.with_tax_amount = sum(self.contract_line_ids.mapped("price_subtotal"))
            rec.tax_amount = sum(self.contract_line_ids.mapped("tax_amount"))
            rec.total_amount = sum(self.contract_line_ids.mapped("amount_without_tax"))
        ...
        for i in contracts:
            if i.date_end:
                ...
                if date_now >= exp_date:
                    mail_content = (
                                "Hello, Mr. " + str(i.user_id.name) + ". Notice about the end date of the contract: " +
                                str(i.name) + " for customer " + str(i.partner_id.name))
    ...
    def cron_invoice_payment(self):
        ...
        for i in invoices:
            if i.invoice_date_due:
                ...
                payment_exp_date = fields.Date.from_string(
                    i.invoice_date_due
                ) - timedelta(days=30)
                if date_now == exp_date:
                    mail_content = ("Hello, Mr. " + i.user_id.name + ". It is time to pay the bill number "
                                    + i.sequence_number_next_prefix + i.sequence_number_next + " for the customer "
                                    + i.partner_id.name + "on " + i.invoice_date_due)
                    main_content = {
                        "subject": _("Contract-%s Expired On %s")
                                   % (i.name, i.invoice_date_due),
                        "author_id": self.env.user.partner_id.id,
                        "body_html": mail_content,
                ...
                if payment_exp_date <= date_now < fields.Date.from_string(i.invoice_date_due):
                    mail_content = (
                            "Hello, Mr. " + i.user_id.name + ". Notice that the payment date of the invoice number  "
                            + i.sequence_number_next_prefix + i.sequence_number_next + " for the customer "
                            + i.partner_id.name + " is approaching on " + i.invoice_date_due
                    )
                    main_content = {
                        "subject": _("Contract-%s Expired On %s")
                                   % (i.name, i.invoice_date_due),
...
class AccountPayment(models.Model):
    ...
    def create(self, vals):
        ...
        if move_id:
            rec.update({"contract_id": move_id.contract_id.id,
                        "installment_id": move_id.installment_id.id})
            # move_id.write({'payment_id': rec.id })
        return rec
