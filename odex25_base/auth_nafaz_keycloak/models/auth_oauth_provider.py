...
class AuthOauthProvider(models.Model):
    ...
    flow = fields.Selection([("access_token", "OAuth2"), ("id_token_code", "OpenID Connect (authorization code flow)"),
                             ("id_token", "OpenID Connect (implicit flow, not recommended)")], string="Auth Flow",
                            required=True, default="access_token")
    ...
    def _parse_id_token(self, id_token, access_token):
        ...
        res.update(jwt.decode(id_token, self._get_key(header.get("kid")), algorithms=["RS256"], audience=self.client_id, access_token=access_token))
