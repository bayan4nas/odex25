...
class ReportController(report.ReportController):
    ...
    def _report_routes_xlsx(self, reportname, docids=None, converter=None, **data):
        try:
            ...
            if data.get("context"):
                # Handle context data and language
                data["context"] = json.loads(data["context"])
                if data["context"].get("lang"):
                    del data["context"]["lang"]
                context.update(data["context"])

            # Render the XLSX report with the updated context and data
            xlsx = report.with_context(context)._render_xlsx(docids, data=data)[0]

            # Customize report name if necessary
            report_name = report.name
            if report.print_report_name and docids:
                obj = request.env[report.model].browse(docids[0])
                report_name = safe_eval(report.print_report_name, {"object": obj})

            # Prepare HTTP headers for response
            xlsxhttpheaders = [
            ...
            # Return the response with the generated XLSX data
