...
class WebsiteHrRecruitmentInherited(WebsiteHrRecruitment):
    ...
    def jobs(self, page=None, country=None, department=None, office_id=None, **kwargs):
        env = request.env(context=dict(request.env.context, show_address=True, no_tag_br=True))
        jobs_per_page = int(env['ir.config_parameter'].sudo().get_param('saip_website_theme.jobs_per_page',default=5))
        ...
        job_ids = Jobs.search(domain, order="is_published desc, no_of_recruitment desc", offset=(page - 1) * jobs_per_page,
                              limit=jobs_per_page).ids
        # Browse jobs as superuser, because address is restricted
        jobs = Jobs.sudo().browse(job_ids)
        # setup pagination
        domain.append(("is_published", "=", True))
        model_ids = Jobs.search(domain, order="is_published desc, no_of_recruitment desc")
        ...
        response = request.render("website_hr_recruitment.index", {
            'jobs': jobs,
            'countries': countries,
